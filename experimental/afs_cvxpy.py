...
In this experimental file, "cvxpy" replaces "mip" for optimization. To use this file,
...
class AlternativeFeatureSelector(metaclass=ABCMeta):
    ...
    # This routine will be called each time before running the solver.
    ...
    # - "constraints": constraints to be added, e.g., on the size of the feature set(s)
    @abstractmethod
    def create_problem(self, s_list: Sequence[Sequence[cvxpy.Variable]],
                       constraints: Sequence[cvxpy.Expression]) -> cvxpy.Problem:
    ...
    @abstractmethod
    def select_and_evaluate(self, problem: cvxpy.Problem,
                            s_list: Sequence[Sequence[cvxpy.Variable]]) -> pd.DataFrame:
        raise NotImplementedError('Abstract method.')

    # Return a variable and the neceessary constraints for the product between two binary variables
    # from the same problem.
    # See https://docs.mosek.com/modeling-cookbook/mio.html#boolean-operators ("AND" operator)
    @staticmethod
    def create_product_var(var1: cvxpy.Variable, var2: cvxpy.Variable) -> Tuple[cvxpy.Variable, Sequence[cvxpy.Expression]]:
        constraints = []
    ...
    # Return constraints such that the dissimilarity between two feature sets of desired size "k"
    # is over a threshold "tau". Several constraints are needed as we need to model product terms.
    # The selection decisions "s2" for the 2nd feature set are unknown
    ...
    def create_pairwise_alternative_constraint(
            s1: Sequence[Union[cvxpy.Variable, int, bool]], s2: Sequence[cvxpy.Variable], k: int,
            tau: Optional[float] = None, tau_abs: Optional[int] = None,
            d_name: str = 'dice') -> Sequence[cvxpy.Expression]:
        assert len(s1) == len(s2), 'Decision vectors s1 and s2 need to have same length.'
        assert isinstance(s2[0], cvxpy.Variable), 's2 needs to contain variables.'
        constraints = []
        if isinstance(s1[0], cvxpy.Variable):  # both features sets undetermined
            product_variables = []
            for (s1_j, s2_j) in zip(s1, s2):
                p_var, p_cons = AlternativeFeatureSelector.create_product_var(s1_j, s2_j)
                product_variables.append(p_var)
                constraints.extend(p_cons)
            overlap_size = cvxpy.sum(product_variables)
        else:  # one feature set known, so plain sum
            overlap_size = cvxpy.sum([s2_j for (s1_j, s2_j) in zip(s1, s2) if s1_j])
        if d_name == 'dice':  # as same size of both sets, also equivalent to some other measures
            if tau_abs is not None:
                constraints.append(overlap_size <= k - tau_abs)
            else:
                constraints.append(overlap_size <= (1 - tau) * k)
        elif d_name == 'jaccard':
            constraints.append(overlap_size <= (1 - tau) / (2 - tau) * 2 * k)
        else:
    ...
    def search_sequentially(self, k: int, num_alternatives: int, tau: Optional[float] = None,
        ...
        results = []
        constraints = []
        s = [cvxpy.Variable(name=f's_{j}', boolean=True) for j in range(self._n)]
        s_list = [s]  # only search for one feature set at a time
        constraints.append(cvxpy.sum(s) == k)  # select exactly k
        problem = self.create_problem(s_list=s_list, constraints=constraints)
        ...
        for _ in range(num_alternatives):
            if not math.isnan(results[-1]['train_objective'].iloc[0]):  # if not infeasible
                ...
                constraints.extend(AlternativeFeatureSelector.create_pairwise_alternative_constraint(
    ...
    def search_simultaneously(self, k: int, num_alternatives: int, tau: Optional[float] = None,
        ...
        for i in range(num_alternatives + 1):  # find "num_alternatives" + 1 feature sets
            s = [cvxpy.Variable(name=f's{i}_{j}', boolean=True) for j in range(self._n)]
...
class LinearQualityFeatureSelector(AlternativeFeatureSelector):
    ...
    def __init__(self):
        ...
        self._Q_train_list = None  # Sequence[cvxpy.Expression]; objectives for the feature sets
        self._Q_test_list = None  # Sequence[cvxpy.Expression]; for evaluation, not optimization
    ...
    # Initialize the problem by creating linear objectives from the features' qualities.
    # See the superclass for a description of the parameters.
    def create_problem(self, s_list: Sequence[Sequence[cvxpy.Variable]],
                       constraints: Sequence[cvxpy.Expression]) -> None:
        self._Q_train_list = [cvxpy.sum([q_j * s_j for (q_j, s_j) in zip(self._q_train, s)])
                              for s in s_list]
        self._Q_test_list = [cvxpy.sum([q_j * s_j for (q_j, s_j) in zip(self._q_test, s)])
                             for s in s_list]
        objective = cvxpy.Maximize(cvxpy.sum(self._Q_train_list))  # sum over all feature sets
    ...
    def select_and_evaluate(self, problem: cvxpy.Problem,
        ...
        if problem.status == 'optimal':
            result = pd.DataFrame({
                'selected_idxs': [[j for (j, s_j) in enumerate(s) if s_j.value] for s in s_list],
...
class FCBFSelector(MISelector):
    ...
    # Initialize the problem by creating linear objectives from the features' qualities and
        ...
        for j_1 in range(1, self._n):  # 1st feature
            for j_2 in range(j_1):  # 2nd feature
                ...
                if ((self._mi_target[j_1] <= self._mi_features[j_1][j_2]) or
                    ...
                    for s in s_list:
                        interaction_constraints.append(s[j_1] + s[j_2] <= 1)
        return super().create_problem(s_list=s_list,
                                      constraints=constraints + interaction_constraints)
...
class GreedyWrapperSelector(AlternativeFeatureSelector):
    ...
    def create_problem(self, s_list: Sequence[Sequence[cvxpy.Variable]],
                       constraints: Sequence[cvxpy.Expression]) -> None:
        return cvxpy.Problem(objective=cvxpy.Maximize(0), constraints=constraints)
    ...
    def select_and_evaluate(self, problem: cvxpy.Problem,
                            s_list: Sequence[Sequence[cvxpy.Variable]]) -> pd.DataFrame:
        start_time = time.process_time()
        problem.solve()  # only check constraint satisfcation
        iters = 1  # solver called for first time
        if problem.status != 'optimal':  # no valid solution
            result = pd.DataFrame({
                'selected_idxs': [[] for _ in s_list],
            ...
            s_value_list = [[bool(s_j.value) for s_j in s] for s in s_list]
            ...
            while (iters < self._max_iters) and (j < self._n):
                swap_constraints = [s[j] == 0 if s_value[j] else s[j] == 1 for (s, s_value)
                                    in zip(s_list, s_value_list)]
                temp_problem = cvxpy.Problem(objective=problem.objective,
                                             constraints=problem.constraints + swap_constraints)
                temp_problem.solve()
                iters = iters + 1
                if temp_problem.status == 'optimal':
                    current_s_value_list = [[bool(s_j.value) for s_j in s] for s in s_list]
        ...
        end_time = time.process_time()
        result['optimization_time'] = end_time - start_time
        result['optimization_status'] = temp_problem.status
        result['iters'] = iters
        return result
