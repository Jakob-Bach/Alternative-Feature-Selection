...
In this experimental file, mRMR is added as feature-selection method (triggering some refactoring of
other classes as well). To use this file, checkout b032860a86 (newer versions might work as well)
and replace "afs.py" with this file.
"""
...
class AlternativeFeatureSelector(metaclass=ABCMeta):
    ...
    def initialize_model(self, model: mip.Model, s_list: Sequence[Sequence[mip.Var]],
                         k: int) -> None:
        for s in s_list:
    ...
    def search_sequentially(self, k: int, num_alternatives: int, tau: Optional[float] = None,
        ...
        self.initialize_model(model=model, s_list=s_list, k=k)
...
class WhiteBoxFeatureSelector(AlternativeFeatureSelector):
    """Feature Selection as White-Box Problem

    (Abstract) white-box feature selector. Subclasses need to define the function for computing the
    objective.
    """
    ...
    # Should return expressions for train objective and test objective, using variables from "s",
    # data stored internally, and potentially the number of features "k". Might add auxiliary
    # variables to the "model".
    @abstractmethod
    def create_objectives(self, model: mip.Model, s: Sequence[mip.Var],
                          k: int) -> Tuple[mip.LinExpr, mip.LinExpr]:
        raise NotImplementedError('Abstract method.')

    # Initialize the "mip" model by creating an expression of the objective for each feature set in
    # "s_list". See the superclass for a description of the parameters.
    def initialize_model(self, model: mip.Model, s_list: Sequence[Sequence[mip.Var]],
                         k: int) -> None:
        super().initialize_model(model=model, s_list=s_list, k=k)
        self._Q_train_list = []
        self._Q_test_list = []
        for s in s_list:
            Q_train, Q_test = self.create_objectives(model=model, s=s, k=k)
            self._Q_train_list.append(Q_train)
            self._Q_test_list.append(Q_test)
...
class LinearQualityFeatureSelector(WhiteBoxFeatureSelector):
    ...
    def create_objectives(self, model: mip.Model, s: Sequence[mip.Var],
                          k: int) -> Tuple[mip.LinExpr, mip.LinExpr]:
        return (mip.xsum(q_j * s_j for (q_j, s_j) in zip(self._q_train, s)),
                mip.xsum(q_j * s_j for (q_j, s_j) in zip(self._q_test, s)))
...
class MRMRSelector(WhiteBoxFeatureSelector):
    ...
    def __init__(self):
        super().__init__()
        self._mi_target_train = None  # np.ndarray; dependency of each feature to target
        self._mi_target_test = None  # np.ndarray; dependency of each feature to target
        self._mi_features_train = None  # Sequence[np.ndarray]; dependency of each to each feature
        self._mi_features_test = None  # Sequence[np.ndarray]; dependency of each to each feature
    ...
    # Create train objective and test objective by considering feature relevance and redundancy.
    # See the superclass for a description of the parameters and the return value.
    def create_objectives(self, model: mip.Model, s: Sequence[mip.Var],
                          k: int) -> Tuple[mip.LinExpr, mip.LinExpr]:
        relevance_train = mip.xsum(q_j * s_j for (q_j, s_j) in zip(self._mi_target_train, s))
        relevance_test = mip.xsum(q_j * s_j for (q_j, s_j) in zip(self._mi_target_test, s))
        redundancy_terms_train = []
        redundancy_terms_test = []
        for j_1 in range(1, self._n):  # 1st feature
            for j_2 in range(j_1):  # 2nd feature
                interaction_var = AlternativeFeatureSelector.create_product_var(s[j_1], s[j_2])
                redundancy_terms_train.append(interaction_var * self._mi_features_train[j_1][j_2])
                redundancy_terms_test.append(interaction_var * self._mi_features_test[j_1][j_2])
                redundancy_terms_train.append(interaction_var * self._mi_features_train[j_2][j_1])
                redundancy_terms_test.append(interaction_var * self._mi_features_test[j_2][j_1])
        return (relevance_train / k - mip.xsum(redundancy_terms_train) / (k * (k - 1)),
                relevance_test / k - mip.xsum(redundancy_terms_test) / (k * (k - 1)))
...
class FCBFSelector(MISelector):
    ...
    See Yu et. al (2003): "Feature Selection for High-Dimensional Data: A Fast Correlation-Based
    Filter Solution" (which does not use constraints, but proposes a search heuristic)
    """
